...
DROP TABLE TRMP_MIGRATE_LOG;
CREATE TABLE TRMP_MIGRATE_LOG
  AS (
SELECT v.view_id ,v.INVENTORY_ID, v.STATUS_ID  , m.SECONDARY_CODE  , m.EXTERNAL_ID , m.INVENTORY_TYPE_ID  ,1 MIGRATE_LOG
from INVD_VIEWS v
left join INVD_MAIN m
on v.INVENTORY_ID = m.INVENTORY_ID);
CREATE INDEX TRMP_MIGRATE_INDEX1 ON TRMP_MIGRATE_LOG (MIGRATE_LOG);


UPDATE TRMP_MIGRATE_LOG SET MIGRATE_LOG = 2 
where MIGRATE_LOG = 1
AND ROWNUM <= 9200000;

UPDATE TRMP_MIGRATE_LOG SET MIGRATE_LOG = 3 
where MIGRATE_LOG = 1
AND ROWNUM <= 9200000;

UPDATE TRMP_MIGRATE_LOG SET MIGRATE_LOG = 4 
where MIGRATE_LOG = 1
AND ROWNUM <= 9200000;

UPDATE TRMP_MIGRATE_LOG SET MIGRATE_LOG = 5 
where MIGRATE_LOG = 1
AND ROWNUM <= 9200000;
