create or replace PROCEDURE MIGRATE_INV_LOG_4 AS 
  ...
  CURSOR C_MAIN_DATA IS
   SELECT ROWID ID_ , view_id ,INVENTORY_ID, STATUS_ID  , SECONDARY_CODE  , EXTERNAL_ID , INVENTORY_TYPE_ID from TRMP_MIGRATE_LOG
   WHERE MIGRATE_LOG = 4
  ...
  init_data invuser.TEST_INV_LOG_NUMBER%ROWTYPE;
  DAT C_MAIN_DATA%ROWTYPE; 
BEGIN
 
   DBMS_OUTPUT.PUT_LINE( 'START MIGRATE_INV_LOG_4 : ' || to_char(sysdate , 'dd/mm/yyyy hh24:mi'));
...
--         DBMS_OUTPUT.PUT_LINE('C_MAIN_DATA : ' || C_MAIN_DATA%ROWCOUNT);
         LOOP
          ...
          INSERT INTO INVUSER.TEST_INV_LOG (ACTION ,BULK_ID ,CREATED_BY ,CREATED_DATE ,EXECUTE_DATE ,EXECUTE_TIME ,ID ,ORDER_ID ,ORDER_STATUS ,TOTAL ,TOTAL_FAIL ,TOTAL_SUCCESS ,UPDATED_BY ,UPDATED_DATE)
          VALUES (2 , NULL , 'Migration' , SYSDATE ,NULL ,NULL , invuser.TEST_INVLOG_SEQ.nextval , V_PREFIX_VIEW_ID||DAT.view_id ,1,1,0,1 ,NULL,NULL  );
          
          
          init_data.ID := INVUSER.TEST_INVLOGNUMBER_SEQ.NEXTVAL;
...
--              DBMS_OUTPUT.PUT_LINE( 'err : ' || DAT.EXTERNAL_ID);
          ...
          INSERT INTO INVUSER.TEST_INV_LOG_NUMBER ( ID, ORDER_ID, MSISDN, ICCID, STATUS, RESULT, REMARK, IMSI, MAPPING_STATUS, MODIFY_DATE, BULK_ID, STATUS_PREVIOUS ) 
          ...
          UPDATE TRMP_MIGRATE_LOG SET MIGRATE_LOG = 9 WHERE ROWID = DAT.ID_;
